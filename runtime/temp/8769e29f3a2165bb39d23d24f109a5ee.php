<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:36:"template/platform\Auth\userList.html";i:1513303780;s:27:"template/platform\base.html";i:1513303780;s:44:"template/platform\controlCommonVariable.html";i:1513303780;s:31:"template/platform\urlModel.html";i:1513303780;s:27:"template/platform\page.html";i:1513303780;s:33:"template/platform\openDialog.html";i:1513303780;}*/ ?>
<!DOCTYPE html>
...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/auth/userlist'); ?>",
			async : true,
			data : {
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						if(data['data'][i]['is_admin'] == 1){
							...
							html += '<td class="center"><a href="'+__URL('PLATFORM_MAIN/auth/edituser?uid=' + data["data"][i]["uid"]) + '">修改</a>&nbsp;&nbsp; ';
            ...
            buttons: {
                "确定,#e57373": function() {
                    ...
                    $.ajax({
            			...
            			success : function(data) {
            				if(data["code"] > 0 ){
            					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/auth/userlist'); ?>");
            				}else{
	...
	function lock(uid){
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/auth/userlock'); ?>",
			async : true,
			data : {
				"uid" : uid,
			},
	...
	function unlock(uid){
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/auth/userunlock'); ?>",
			async : true,
			data : {
				"uid" : uid,
			},
